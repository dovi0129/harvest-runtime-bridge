...
# configure_sensor(profile.get_device())
...
def main():
    ...
    # [PATCH] 로봇 바쁨 제어 & ACK 수신(UDP 9998)
    robot_ready = True
    ACK_PORT = 9998
    ack_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ack_sock.bind(("0.0.0.0", ACK_PORT))
    ack_sock.setblocking(False)
    print(f"[ACK] Listening on UDP :{ACK_PORT}")
    ...
    try:
        while True:
            ...
            if preds is None or len(preds) == 0:
                # [PATCH] ACK 폴링은 프레임마다 수행
                try:
                    data, _ = ack_sock.recvfrom(128)
                    ack_ok = (data == b"ACK_DONE")
                    if not ack_ok:
                        try:
                            j = json.loads(data)
                            ack_ok = (j.get("ack") == "done")
                        except Exception:
                            ack_ok = False
                    if ack_ok:
                        robot_ready = True
                        print("[ACK] done received. ready for next target.")
            ...
            for inst_id, cx, cy in instance_centers:
                mask_i = (instance_mask == inst_id)
                maturity = classify_strawberry_maturity(hsv_image, mask_i)
                if maturity != 'fully_ripe':
                    continue

                tip, midpoint, picking_pts = extract_centerline_and_picking_points(mask_i.astype(np.uint8))
                if tip is None or midpoint is None or len(picking_pts) != 2:
                    continue

                angle = compute_angle(tip, midpoint)
                depth_value = get_mean_valid_depth_in_mask(depth_frame, mask_i.astype(np.uint8), padding=6)
                depth_text = 'N/A'
                if depth_value is not None:
                    ...
                    # [PATCH] 전송 게이트: 로봇이 준비됐을 때만 1회 전송
                    if robot_ready:
                        sender.send_data(message)
                        robot_ready = False
                        sent_this_frame = True
                        print("[SEND] target sent. waiting ACK...")
                    else:
                        print("[WAIT] robot busy; skip sending this frame")
                else:
            ...
            # [PATCH] ACK 논블로킹 수신 (프레임 종료 시점에도 체크)
            try:
                data, _ = ack_sock.recvfrom(128)
                ack_ok = (data == b"ACK_DONE")
                if not ack_ok:
